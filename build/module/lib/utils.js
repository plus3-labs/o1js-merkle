 ...
 * @returns
 */
export const bufferToInt256 = (buf) => {
    const bi = BigInt("0x" + buf.toString("hex"));
    const res = bi & ((BigInt(1) << BigInt(256)) - BigInt(1));
    return res;
};
export function separateHighPartFor254BitField(x) {
    let { xDiv2Var, isOddVar } = Provable.witness(provablePure({ xDiv2Var: Field, isOddVar: Field }), () => {
        let bits = x.toBits();
 ...
 * @returns
 */
export function greaterThanFor254BitField(x, y) {
    let { xDiv2Var: xHighPart, isOddVar: xParity } = separateHighPartFor254BitField(x);
    let { xDiv2Var: yHighPart, isOddVar: yParity } = separateHighPartFor254BitField(y);
    Provable.log('greaterThanFor254BitField - xHighPart: ', xHighPart, ', xParity: ', xParity);
    Provable.log('greaterThanFor254BitField - yHighPart: ', yHighPart, ', yParity: ', yParity);
...
export function fieldArrayToStringArray(fields) {
